# Optimizer call map — single-analyzer refactor (updated 2026-09-06)

Shows the current state of the optimizer layer in upstream `main` after PR #34 merged.
Base for PR #2 planning.
...
All signatures and bodies below match the current upstream `main` exactly — verified
against `b848b19c`.
...
```
BEFORE (pre-PR #34):  AnalyzerResults []NamedAnalyzerResult   (slice — one entry per analyzer)
AFTER  (current):     CompositeSignal  NamedAnalyzerResult    (single value — sat entry from engine)
```
...
```
BEFORE (pre-PR #34):  type RolePairedState []map[string]float64   // [analyzer-index][role] → demand
AFTER  (current):     type RolePairedState map[string]float64     // [role] → demand
```
...
```
BEFORE (pre-PR #34):
    s := []NamedAnalyzerResult{req.AnalyzerResults[0]}
