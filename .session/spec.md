...
Status: v8 — CT1a, CT1b, CT2, CT3a, CT3b, CT5, CT6 are DONE. **CT6 has one outstanding gap:**
its commit (`f20e06f9`) never updated 6 test-file call sites for the `runAnalyzersAndScore`
return-type change — see CT6's Status section. CT7 (engine-side reduce, née "PR #2") is
NOT STARTED, blocked on Q1-Q4 below. **CT4 remains BLOCKED** on the user's fairness-definition
decision (ledger §36) and is explicitly out of scope for the current implementation pass.
Supersedes `spec-compose-analyzer-results.md` (T1's originally-rejected insertion point; kept
for history).
...
The following definitions ground all CT6 reasoning. Every formula in the spec
maps to one of these symbols.

**Entities.**
- Analyzers: A1, A2, … (currently: saturation only)
- Roles: R1, R2, … (prefill, decode, or the synthetic "both")
- Models: M1, M2, … (ScaledObject groups; one model may have multiple SOs)
- Scaled Objects: SO1, SO2, … (one SO per variant, one role per SO)
- GPU types: G, G', …

**Per-SO quantities.**
```
M(SO)          model served by SO
R(SO)          role of SO
N(SO)          current replica count of SO
G(SO)          GPUs of type G needed per replica of SO
```

**Analyzer signals (per analyzer, model, role).**
```
D(A, M, R)     demand attributed to role R of model M by analyzer A
               (token-units for saturation; unitless for a coverage analyzer)
PRC(A, SO)     per-replica capacity of SO as seen by analyzer A (same unit as D)
```

For SO we write `D(A, SO)` as shorthand for `D(A, M(SO), R(SO))`.

**Derived quantities (single analyzer A, single SO).**
```
N_full(A, SO) = ceil( D(A, SO) / PRC(A, SO) )   replicas for full coverage
C(A, SO)      = PRC(A, SO) / D(A, SO)            per-replica coverage fraction ∈ (0,1]
```

Identity: `N_full(A, SO) × C(A, SO) = 1` (up to ceiling).

**Multi-analyzer reduce (engine-side, future CT).**
Replica counts and coverage from different analyzers are comparable per-SO:
```
N_full(SO) = max_i { N_full(A_i, SO) }    take the most-demanding estimate
C(SO)      = min_i { C(A_i, SO) }         take the least-covering estimate
```

**Model-level coverage (combining SOs of the same model).**

Same role (SO1, SO2 have R(SO1) = R(SO2)):
```
C(SO1 + SO2) = C(SO1) + C(SO2)            capacities add linearly
C(M, R)      = Σ_{SO: R(SO)=R} C(SO)
```

Across roles (disaggregated model with prefill and decode roles):
```
C(M) = min( C(M, prefill), C(M, decode) ) + C(M, both)
```
The `both` term covers non-disaggregated replicas; a purely disaggregated model
has `C(M, both) = 0`.

**GPU demand.**
```
NG(SO)   = N_full(SO) × G(SO)    GPUs needed for full coverage of SO
NG(M, R) = Σ_{SO: R(SO)=R} NG(SO)
NG(M)    = Σ_R NG(M, R)          total GPU demand across all roles
```

**Activation threshold.**
`ACT(SO)` is a scale factor (e.g. 1.1 for 10% head-room). The optimizer scales to
`N_full(SO) / ACT(SO)` replicas (i.e. it targets `C(SO) × ACT(SO) = 1`), not raw
`N_full`. All coverage fractions used by the optimizer are relative to this
threshold-adjusted demand. For this spec, `ACT = 1` (threshold absorbed into RC/SC
computation by `applyUniversalThreshold`; see RC/SC section below).

---

#### Coverage normalization (the CT6 transformation)
...
```
C(SO) = PRC(A, SO) / D(A, M(SO), R(SO))
```

The composite signal expresses everything in unit-less coverage terms:
- `TotalDemand = 1.0` (all demand = 100%)
- `RoleDemand[role] = 1.0` for every role (per-role demand = 100%)
- `VariantCapacity.PerReplicaCapacity = C(SO)` (dimensionless fraction in (0, 1])

This preserves the optimizer's `ceil(demand / PRC)` replica-count math exactly:
`ceil(1.0 / C(SO))` = `N_full(SO)` replicas needed for full coverage.
...
| `rescale.go:rescaleInputs` (line 564) | `Demand: satNamed.Result.TotalDemand` | `Demand: SatDemand` (pre-normalization token value saved before conversion runs; see rescale fairness section) |
...
**Current code (`rescale.go` lines 55–66, 561–567).**

`rescaleInputsForGroup` constructs a `rescaleInput` per model with:
```
Demand = satNamed.Result.TotalDemand   // raw tokens from saturation analyzer
CapGPUs = modelDemandGPUs(...)         // ceil(D/PRC) × G — GPUs for full coverage
```

`computeRescaleTargets` water-fills a GPU budget with weight `priority × Demand`,
capping each model at `CapGPUs`:
```
target_i = floor_i + pool × (priority_i × Demand_i) / Σ_j(priority_j × Demand_j)
           subject to: target_i ≤ CapGPUs_i
```
Models that hit their cap return excess to the pool for re-splitting.

**Intended fairness semantics.**

The weight should be proportional to total demand in **replicas** — `N_full(SO)` — not in
tokens and not in GPUs. The intuition: equal-priority models should receive GPU deltas
in proportion to how many replicas they need for full coverage. GPU-per-replica cost (G(SO))
is a hardware property, not a demand signal, and should not inflate a model's share.

Formally, for equal priority, fair split of spare GPUs `B` between two models:
```
DNG(M1) / N_full(M1)  =  DNG(M2) / N_full(M2)
   ⟹  DNG(Mi) ∝ N_full(Mi)  =  NG(Mi) / G(Mi)   (where G is GPUs/replica)
```

**Why `TotalDemand` (tokens) works today — and why CT6 breaks it.**

Currently `Demand = TotalDemand` in tokens. This is proportional to `N_full` only when
`PRC` is comparable across models (same token-capacity per replica). For a homogeneous
cluster this holds well enough. After CT6 `TotalDemand = 1.0` for every model — the
proportionality is destroyed and the weight collapses to priority-only, which is wrong.

**Resolution — two-tier fix.**

*Immediate (CT6):* save `TotalDemand` from the raw sat signal **before** normalization
and store it as a separate field on `NamedAnalyzerResult` (e.g. `SatDemand float64`).
`rescaleInputsForGroup` reads `SatDemand` instead of `Result.TotalDemand`. This is
numerically identical to today (same token value), preserves the current behavior exactly,
and is safe: tokens-proportional-to-replicas holds for a homogeneous deployment.

*Longer-term (future CT):* replace the weight with `N_full(M) = NG(M) / G̅` (replicas
needed for full coverage, GPU-independent). `NG(M) = modelDemandGPUs(...)` is already
computed at `rescale.go:552`; dividing by average G per replica gives the replica-demand
weight. This is the principled fix for heterogeneous deployments and should be a
separate, well-tested change once the semantic framework is established. Tracked as
a follow-on task (not CT6 scope).
...
- [x] **Pre-implementation:** resolve the `rescaleInput.Demand` open decision. **RESOLVED
  2026-09-01** — immediate fix: add `SatDemand float64` field to `NamedAnalyzerResult`,
...
- [ ] Unit test for `normalizeToCompositeUnits`:
  ...
  - `PRC=0` case: `PRC` stays 0 after normalization (not a div-by-zero; the guard is on
    `demand`, not `PRC`).
- [x] Run `go test ./internal/engines/...` — all pass, zero regressions, 3 pre-existing
  skips unchanged.
...
**Status.** DONE — commit `f20e06f9` ("CT6: normalize saturation demand to coverage units
before optimizer"). `SatDemand` field, `normalizeToCompositeUnits`, the `rescaleInputsForGroup`
...
(vacuous pass-throughs), but never updated 6 test-file call sites that still treat the
return value as a single struct — this does not compile as committed. The fix has existed
only as uncommitted working-tree changes since 2026-08-31 (verified via `go build ./...` /
`go vet` passing only with the fix applied); `origin/single-analyzer` carries `f20e06f9`
without it, so origin's HEAD does not build either. `engine_v2_compose_test.go` (which tested
the now-deleted `composeAnalyzerResults`) moved to
`internal/engines/allocation/multi_backup/` (`//go:build ignore`) as part of the same fix.
This needs a commit before any further PR work — see STATE.md "PR history".

---

### CT7 — Engine-side reduce: combine N>1 analyzer results into one composite (a.k.a. "PR #2")

**Intent.** Ported from `.session/ledgers/compose-reduce-design-2026-08-30.md` (archived
2026-09-06 — see note below). Before CT3b/CT6, `composeAnalyzerResults` was a stub that always
returned saturation's result unchanged, discarding every other enabled analyzer's output.
CT3b/CT6 then **deleted** `composeAnalyzerResults`/`rawAnalyzerResult` entirely (they were
vacuous pass-throughs once the optimizer only ever consumed saturation). Implementing this
task therefore does not mean "filling in the stub" — it means reintroducing a real reduce step
at the engine→composite boundary. Pre-CT3b originals (the last version that took a slice of
analyzer results) are preserved verbatim under `internal/engines/allocation/multi_backup/`
(`//go:build ignore`) for reference.

**The invariant.** Never produce a composite result with less coverage than saturation alone
would give. Saturation is the floor: a composite must be at least as aggressive on scale-up,
and at least as conservative on scale-down, as sat's result would have been alone.

**Why analyzer outputs can't just be summed/averaged.** Each analyzer returns `TotalDemand`,
`RoleDemand`, and `VariantCapacities[].PerReplicaCapacity` in **its own units** — saturation in
tokens, a throughput analyzer in tokens/sec, an SLO analyzer in latency-capacity. These are not
the same currency and cannot be added or averaged directly. Saturation always owns PRC (the
only analyzer that reliably produces a meaningful PRC in pod-replica units; a non-sat
analyzer's PRC is in incommensurable units and must not replace sat's).

**The reduce operation.** The common currency is implied replica count:
```
implied_replicas(analyzer_i, variant_v) = ceil( demand_i[v] / PRC_i[v] )
```
This is unit-free. For each variant, the composite takes the **max** implied replica count
across all informative analyzers, then back-converts to saturation's demand units:
```
...
always — the floor invariant holds by construction. Same logic per role (`RoleDemand`) for
disaggregated models.

**Sat-only fast path.** When saturation is the only entry — today's default and the only
production case — the reduce returns sat's result unchanged. Zero behavioral change from
today; this is the invariant CT1-CT6 already hold and must continue to hold once this task
lands.

**Open questions — must be answered before writing code.**
- **Q1 (per-variant `TotalDemand` consistency):** the capacity builder uses `TotalDemand` at
  both model level (`AnalyzerResult.TotalDemand`) and per-variant level
  (`VariantCapacity.TotalDemand`, for utilization display). If model-level `TotalDemand` is
  recomputed as `max over variants`, does per-variant `TotalDemand` on the composite also get
  raised to `max_replicas[v] × sat.PRC[v]` (consistent utilization figures), or does it stay
  as sat's own value for display/diagnostic purposes while only the model-level figure is
  raised?
- **Q2 (`RoleDemand` when a non-sat analyzer doesn't emit one):** sat emits `RoleDemand` keyed
  by role for disaggregated models. A non-sat analyzer might emit only a model-level
  `TotalDemand` with no `RoleDemand`. Does that analyzer's demand participate in the per-role
  max at all, and if so how?
- **Q3 (composite name vs. `isSaturationResult`):** the composed entry's `Name` propagates to
  `CompositeSignal.Name`. `isSaturationResult` (`engine_v2.go:747`) checks this name to decide
  whether to apply a specific rescale path. If the composite is named `"composite"` instead of
  `"saturation"`, that check fails when non-sat analyzers are active, potentially bypassing the
  rescale path unintentionally. Does the composite keep the name `"saturation"` when sat is the
  floor contributor, or does `isSaturationResult` change to check something else (e.g. whether
  sat's result was the floor contributor, not the name)?
- **Q4 (composite `Score`):** `Score` (the fair-share weight) is set by `buildNamedResult` from
  `AnalyzerScoreConfig`. What should the composite's `Score` be — saturation's score (simplest,
  preserves today's behavior), the max score across contributing analyzers, or a fixed value
  (e.g. `1.0`, since the composite is already a unified signal)?
...
- CT4's fairness-definition question — orthogonal to this.

**Todo.**
- [ ] Get Q1-Q4 answered/decided with the user before writing any code
- [ ] Design the actual reduce function (replacing the deleted `composeAnalyzerResults`),
  referencing `multi_backup/` originals where useful, honoring the floor invariant and the
  sat-only fast path
- [ ] Implement, with unit tests covering: sat-only (must be a no-op vs. today), sat + one
  other analyzer (floor invariant holds), disaggregated roles, and each open question's
  resolved behavior
- [ ] Restore/rewrite the 3 currently-`Skip()`-ed multi-analyzer tests (see
  `.session/ledgers/single-analyzer-call-map-2026-08-30.md` §8 for exact locations) per the
  phase-2 plan — they were skipped specifically pending this task, not abandoned
- [ ] Run full test suite — zero regressions on the sat-only path; new tests pass for N>1

**Refs.**
*Archived design source:* `.session/ledgers/compose-reduce-design-2026-08-30.md` (session
capture — content above is the durable version; the archived file is kept for the session
history only, not as a live reference).
*Reads:* `internal/engines/allocation/multi_backup/` (pre-CT3b originals),
`internal/engines/steadystate/engine_v2.go` (`runAnalyzersAndScore`, `collectV2ModelRequest`,
`isSaturationResult`)
*Writes:* TBD once Q1-Q4 are resolved and the design is written.

**Status.** NOT STARTED. Blocked on Q1-Q4. Also blocked, practically, on CT6's outstanding
test-fix gap being committed first (see CT6's Status section) since this task builds on top
of `runAnalyzersAndScore`'s current (slice-returning) shape.
...
- **v6** (2026-09-01, session s8, not previously recorded here): CT6 semantic framework and
  rescale-fairness sections added; CT6 implemented and committed (`f20e06f9`).
- **v7** (2026-09-01, not previously recorded here): CT6 marked DONE was never actually written
  back into this spec — the Status line and Todo checkboxes stayed at "NOT STARTED" despite the
  commit landing. Corrected in v8.
- **v8** (2026-09-06): corrected CT6's stale "NOT STARTED" status to DONE and checked off its
  Todo; recorded CT6's outstanding test-fix gap (found this session — 6 test files never
  updated for `runAnalyzersAndScore`'s slice return type, uncommitted since 2026-08-31). Added
  CT7 (engine-side reduce / "PR #2"), porting the design and Q1-Q4 open questions from
  `compose-reduce-design-2026-08-30.md` before that file was archived to `.session/ledgers/` —
  it was a session capture masquerading as a design doc; the design itself now lives here.
  Renamed this file from `spec.md.wip` to `spec.md` (no live concurrent editor; the `.wip`
  suffix was stale). Archived `spec.md.local` (a pre-CT6 snapshot, superseded) to
  `.session/ledgers/`.
